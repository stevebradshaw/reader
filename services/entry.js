...
function updateEntry(params) {
  q = connection.query("UPDATE user_entry_status SET status = ? where entry_key = ? and user_id = ?",
                       [ "R", params.key, 1 ], 
                       function(err,result) {
                         res.end() ;
                       });  
...
function updateFeed(params) {
  q = connection.query("UPDATE user_entry_status SET status = ? where feed_id = ? and user_id = ?",
                       [ "R", params.feedid, 1 ],
                       function(err,result) {
                         res.end() ;
                       });  
